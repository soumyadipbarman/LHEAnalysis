...
/*
  c++ -o unit09 `root-config --glibs --cflags` \
     -lm hFactory.cc hChain.cc h2Factory.cc h2Chain.cc unit09.cpp
*/
...
bool isSignal (TLorentzVector & fquarkMin, TLorentzVector & fquarkMax, 
...
//          fabs (fquarkMin.Eta () - fquarkMax.Eta ()) > 1.5 &&
          fabs (fquarkMin.Eta () - 0.5 * (fquarkMin.Eta () + fquarkMax.Eta ())) > 0.6 &&
          fabs (fquarkMax.Eta () - 0.5 * (fquarkMin.Eta () + fquarkMax.Eta ())) > 0.6 &&
...
//PG --------------- S I G N A L   D E F I N I T I O N  -------------------
      ...
      //PG histograms for signal and irreducible background together     
...
//PG --------------- S A M P L E   S E L E C T I O N  -------------------
      ...
      if (isSignal (fquarkMin, fquarkMax, lepMin, lepMax, vCand, zCand))
        {
...
//          std::cerr << "bkg not two quarks in the final state\n" ;
  ...
  std::cout << "ALL NUMBERS IN pb\n"
            ...
            << "SELECTED BKG         " << selNONVBFnumber * norm40 + VBFBKGnumber * norm22 << " pb"
...
//PG --------------- D R A W I N G ---------------------------
  ...
  H1fact.applyToAll (scaleH1 (3, norm22)) ;
  H1fact.applyToAll (scaleH1 (4, norm22)) ;

  H2fact.applyToAll (scaleH2 (0, norm40)) ;
  H2fact.applyToAll (scaleH2 (1, norm40)) ;
  H2fact.applyToAll (scaleH2 (2, norm40)) ;
  H2fact.applyToAll (scaleH2 (3, norm22)) ;
  H2fact.applyToAll (scaleH2 (4, norm22)) ;
